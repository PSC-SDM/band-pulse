...
// ---------------------------------------------------------------------------
// Mapper: Ticketmaster event → Domain Event
// ---------------------------------------------------------------------------

function mapTmEventToDomain(
    tmEvent: TmEvent,
    artistId: string,
    artistName: string
): Event {
    ...
    if (tmStatus === 'offsale' && eventDate > now) {
        // Future event that's off sale = very likely sold out
...
function mapTmVenue(venue?: TmVenue): Event['venue'] {
    if (!venue) {
        return {
            name: 'TBA',
            address: '',
